...
let parabolaAnimation = function(element, ops) {
  ...
  if (this.options.className !== '') {
    this.runElement.className = this.options.className;
  }
...
parabolaAnimation.prototype = {
  ...
  initSettings: function() {
    ...
    let vertex_top = Math.min(startPos.top, endPos.top) - Math.abs(startPos.left - endPos.left) / 4;
    if (vertex_top < settings.vertex_Rtop) {
      vertex_top = Math.min(settings.vertex_Rtop, Math.min(startPos.top, endPos.top));
    }

    let distance = Math.sqrt(
      Math.pow(startPos.top - endPos.top, 2) + Math.pow(startPos.left - endPos.left, 2)
    );
    let steps = Math.ceil(
      Math.min(Math.max(Math.log(distance) / 0.05 - 75, 30), 100) / settings.speed
    );
    let ratio =
      startPos.top === vertex_top
        ? 0
        : -Math.sqrt((endPos.top - vertex_top) / (startPos.top - vertex_top));
    let vertex_left = (ratio * startPos.left - endPos.left) / (ratio - 1);
    let curvature =
      endPos.left === vertex_left
        ? 0
        : (endPos.top - vertex_top) / Math.pow(endPos.left - vertex_left, 2);
